...
class Purchase extends BaseController
{
    ...
    {   
        ...
        $this->view['customers'] = $this->partyModel->select('party.id,party.party_name')
        ...
        $this->view['last_order'] = $this->POModel->orderBy('id', 'desc')->first();
        $this->view['branches'] = $this->selectUserBranches(); 
        return view('Purchase/create', $this->view);
    ...
    public function edit($id)
    {
        ...
        $this->view['last_order'] = $this->POModel->orderBy('id', 'desc')->first();
        $this->view['order_details'] = $this->POModel->where('id',$id)->orderBy('id', 'desc')->first();
        $this->view['selected_customer'] = $this->view['order_details']['customer_name'];
        if(!empty($this->view['order_details']['customer_name'])){
        ...
        $this->view['branches'] = $this->selectUserBranches();
        return view('Purchase/edit', $this->view);  
    }

    function checkOrderStatus($id){
        //order can be edited only 2 times as per doc
        $this->view['order_details'] = $this->POModel->where('id', $id)->first();
        ...
        if(isset($this->view['order_details']['edit_count']) && $this->view['order_details']['edit_count'] < 2){
    ...
    public function addProducts($id)
    {
        ...
        $this->view['order_details'] = $this->POModel->where('id', $id)->first();
        $this->view['added_products'] = $this->POPModel->select('*,purchase_order_products.id as sp_id')->join('products', 'products.id = purchase_order_products.product_id')->where('order_id', $id)->findAll();

        return view('Purchase/addProducts', $this->view);
    }
    ...
    public function getProducts()
    {
        ...
        return view('Purchase/productTable', $this->view);
    }

    public function showWebCam()
    { 
        $this->view['data'] =  $this->request->getPost(); 
        return view('Purchase/webCam',$this->view);
    }
    ...
    function purchaseCheckout($orderId){
        ...
        $this->view['order_details'] = $this->POModel->where('id', $orderId)->first();
        $this->view['added_products'] = $this->POPModel->select('*,purchase_order_products.id as sp_id')->join('products', 'products.id = purchase_order_products.product_id')->where('order_id', $orderId)->findAll();
        return view('Purchase/purchaseCheckout', $this->view);
    }
    ...
    function view($orderId){
        ...
        $this->view['order_details'] = $this->POModel->where('id', $orderId)->first();
        $this->view['added_products'] = $this->POPModel->select('*,purchase_order_products.id as sp_id')->join('products', 'products.id = purchase_order_products.product_id')->where('order_id', $orderId)->findAll();
        return view('Purchase/view', $this->view);
    }

    function purchaseCheckoutPrint($orderId){
        $this->view['token'] = $orderId;
        $this->view['order_details'] = $this->POModel->select('purchase_orders.*,p.business_address,p.primary_phone')
        ->join('party p', ' p.id = purchase_orders.party_id','left')->where('purchase_orders.id', $orderId)->first();
        
        $this->view['added_products'] = $this->POPModel->select('*,purchase_order_products.id as sp_id,u.unit')
        ...
        $this->view['total_quantity_per_units'] = $this->POPModel->select('products.product_name product_name,sum(purchase_order_products.quantity) as sop_quantity,u.unit as unit')
        ->join('products', 'products.id = purchase_order_products.product_id')
        ->join('units u', 'u.id = products.unit_id','left')
        ...
        return view('Purchase/purchaseCheckoutPrint', $this->view);
    }
